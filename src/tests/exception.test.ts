...
import {
  getCaptureExceptionMetadata,
  getExceptionMessage,
  getExceptionMessageFromEvent,
} from '../main/modules/exception.js'
...
describe('getCaptureExceptionMetadata', () => {
  test('adds ErrorBoundary context and automatic route fields', () => {
    const currentLocation = {
      href: 'https://app.example.com/orders/1',
      pathname: '/orders/1',
    } as Location
    expect(
      getCaptureExceptionMetadata({ componentStack: 'at OrderPage' }, currentLocation),
    ).toEqual({
      mechanism: 'react.error_boundary',
      handled: true,
      level: 'error',
      componentStack: 'at OrderPage',
