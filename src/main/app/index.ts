      ...
      if (batchSize > this.compressionThreshold && hasCompressionAPI) {
        ...
        new Response(stream)
          ...
          .then((compressedBuffer) => {
            const compressedBatch = new Uint8Array(compressedBuffer)
            if (compressedBatch[0] === 0x1f && compressedBatch[1] === 0x8b) {
              ...
              this.debug.error('Openreplay compression error: invalid gzip header')
              this.worker?.postMessage({ type: 'uncompressed', batch, batchNum })
            }
          })
          .catch((err) => {
