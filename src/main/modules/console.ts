...
import { getExceptionMessage, reportedErrors } from './exception.js'
...
export interface Options {
  ...
  captureConsoleExceptions: boolean
}
...
export default function (app: App, opts: Partial<Options>): void {
  ...
  const sendConsoleException = app.safe((args: unknown[]): void => {
    for (const arg of args) {
      if (arg instanceof Error && !reportedErrors.has(arg)) {
        reportedErrors.add(arg)
        const msg = getExceptionMessage(arg, [], { source: 'console' })
        app.send(msg)
        break
      }
    }
  ...
  const patchConsole = (console: Console, ctx: typeof globalThis) => {
    const handler = {
      apply: function (target: Console['log'], thisArg: typeof this, argumentsList: unknown[]) {
        ...
        if (options.captureConsoleExceptions && target.name === 'error') {
          sendConsoleException(argumentsList)
        }
        n = n + 1
