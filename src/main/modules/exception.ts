...
export const reportedErrors = new WeakSet<Error>()
...
export function getCaptureExceptionMetadata(
  context: CaptureExceptionContext = {},
  currentLocation: Location | null = typeof location === 'undefined' ? null : location,
  ...
  if (context.componentStack) {
    metadata.componentStack = context.componentStack
  }
  if (currentLocation) {
    metadata.url = currentLocation.href
    metadata.route = currentLocation.pathname
  }
  return metadata
}
    ...
    function handler(e: ErrorEvent | PromiseRejectionEvent): void {
      const error =
        e instanceof ErrorEvent
          ? e.error
          : 'PromiseRejectionEvent' in context && e instanceof context.PromiseRejectionEvent
            ? e.reason
            : null
      ...
      const metadata = {
        mechanism: e instanceof ErrorEvent ? 'window.onerror' : 'unhandledrejection',
        handled: false,
        url: context.location?.href,
        route: context.location?.pathname,
      }
      const msg = getExceptionMessageFromEvent(e, context, metadata)
      if (msg != null) {
        app.send(msg)
